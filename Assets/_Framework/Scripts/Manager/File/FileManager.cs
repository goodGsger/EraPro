...
namespace Framework
{
    public class FileManager : Manager, IFileManager
    {
                            ...
                            switch (data.action)
                            {
                                case FileAction.save:
                                    if (data.isDirectory == false)
                                        FileHelper.WriteFile(path, data.bytes, data.overwrite);
                                    break;
                                ...
                                case FileAction.copy:
                                    if (data.destPath != null)
                                    {
                                        if (data.isDirectory)
                                            ...
                                            FileHelper.CopyFile(path, data.destPath, data.overwrite);
                                    }
                                    break;
                            }
                        }
        ...
        {
            return FileHelper.CopyFile(App.pathManager.streamingAssetPathFile + sourcePath, App.pathManager.persistentDataPathFile + destPath, overwrite);
        }
        ...
        /// <param name="overwrite"></param>
        /// <returns></returns>
        public bool CopyFileStreamingAsync(string sourcePath, string destPath, bool overwrite)
        {
        ...
        {
            return FileHelper.WriteFile(App.pathManager.persistentDataPathFile + path, bytes, overwrite);
        }
        ...
        /// <param name="overwrite"></param>
        /// <returns></returns>
        public bool WriteFilePersistentAsync(string path, byte[] bytes, bool overwrite)
        {
            FileData fileData = new FileData();
        ...
        {
            return FileHelper.WriteText(App.pathManager.persistentDataPathFile + path, text, overwrite);
        }
        ...
        /// <param name="overwrite"></param>
        /// <returns></returns>
        public bool WriteTextPersistentAsync(string path, string text, bool overwrite)
        {
            FileData fileData = new FileData();
            fileData.action = FileAction.save;
            fileData.isDirectory = false;
        ...
        {
            return FileHelper.DeleteFile(App.pathManager.persistentDataPathFile + path);
        }
        ...
        /// <param name="path"></param>
        /// <returns></returns>
        public bool DeleteFilePersistentAsync(string path)
        {
            FileData fileData = new FileData();
        ...
        {
            return FileHelper.MoveFile(App.pathManager.persistentDataPathFile + sourcePath, App.pathManager.persistentDataPathFile + destPath);
        }
        ...
        /// <param name="destPath"></param>
        /// <returns></returns>
        public bool MoveFilePersistentAsync(string sourcePath, string destPath)
        {
            FileData fileData = new FileData();
        ...
        {
            return FileHelper.CopyFile(App.pathManager.persistentDataPathFile + sourcePath, App.pathManager.persistentDataPathFile + destPath, overwrite);
        }
        ...
        /// <param name="overwrite"></param>
        /// <returns></returns>
        public bool CopyFilePersistentAsync(string sourcePath, string destPath, bool overwrite)
        {
            FileData fileData = new FileData();
            fileData.action = FileAction.copy;
            fileData.isDirectory = false;
        ...
        {
            return FileHelper.DeleteDirectory(App.pathManager.persistentDataPathFile + path);
        }
        ...
        /// <param name="path"></param>
        /// <returns></returns>
        public bool DeleteDirectoryPersistentAsync(string path)
        {
            FileData fileData = new FileData();
            fileData.action = FileAction.delete;
        ...
        {
            string persistentPath = App.pathManager.persistentDataPathFile + path;
            if (FileHelper.FileExists(persistentPath))
                return true;

            string streamingPath = App.pathManager.streamingAssetPathFile + path;
            if (FileHelper.FileExists(streamingPath))
                return true;
