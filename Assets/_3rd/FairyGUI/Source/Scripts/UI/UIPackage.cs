...
namespace FairyGUI
{
    ...
    public class UIPackage
    {
        ...
        // FairyGUI修改：添加聚合UI布局加载方式
        public static UIPackage AddPackage(string mainAssetName, byte[] source, AssetBundle res, bool unloadBundleAfterLoaded)
        {
        ...
        public static UIPackage AddPackage(string assetPath, UIPackage.LoadResource loadFunc)
        {
            ...
            // FairyGUI修改：改变读取文本方式
            byte[] bytes = null;
            object asset = loadFunc(assetPath, ".bytes", typeof(TextAsset));
            if (asset is byte[])
                bytes = asset as byte[];
            else if (asset == null)
            {
                ...
                bytes = ((TextAsset)asset).bytes;

            UIPackage pkg = new UIPackage();
            pkg.Create(bytes, null, assetPath, loadFunc, false);
        ...
        {
            // FairyGUI修改：添加容错
            if (param != null && param is AssetBundle)
            {
        ...
        IEnumerator _CreateObjectAsync(PackageItem item, CreateObjectCallback callback)
        {
            ...
            for (int i = 0; i < cnt; i++)
            {
                ...
                if ((i % UIConfig.createObjectAsyncCount == 0) && Time.realtimeSinceStartup - t >= frameTime)
                {
                    yield return null;
        ...
        void LoadAtlas(PackageItem item)
        {
            ...
            else
            {
                ...
                if (tex != null)
                {
                    ...
                    // FairyGUI修改：修改资源读取方式
#if UNITY_EDITOR || UNITY_STANDALONE_WIN
                    item.texture.alphaTexture.storedODisk = false;
#else
